...
interface Props {
    seSelectedInvoices: React.Dispatch<React.SetStateAction<Invoice[] | undefined>>
}

const formSchema = z.object({
    type: z.string().min(1, { message: "Type is required!" }),
    price: z.string().min(1).max(8000),
    date: z.date().max(new Date(), "date shouldn't be in the future"),
    remark: z.string().min(1),
});

const InvoiceCreationSheep = ({seSelectedInvoices} : Props) => {
    ...
    function addInvoice(values: z.infer<typeof formSchema>) {
        seSelectedInvoices(prev => prev?.concat(
            {
                type: values.type,
        ...
        toast.success("Invoice has been created", {
            description: "Type: " + values.type + " - Price: " + values.price,
            action: {
              label: "Close",
